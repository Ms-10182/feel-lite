...
const generateOtp = asyncHandler(async (req, res) => {
  if (!req.user) {
    throw new ApiError(403, "unauthorized access at generateOtp");
  }
  ...
  if (lastOtp) {
    ...
    await Otp.deleteMany({ owner: new mongoose.Types.ObjectId(req.user._id) });
  }
  ...
  const userEmail = await User.findById(req.user._id);
  // After OTP is created
  await sendEmail({
    to: userEmail.email, // make sure you fetch user's email
...
export { generateOtp,  changePassword,
  updateAccountDetails, deleteAccount };
